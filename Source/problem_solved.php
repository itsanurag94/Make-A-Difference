...
if($problem_status == 'solved')
{
	if($num_rows == 0)
	{
	$query_1 = "INSERT INTO Problem_solved(pID, verified_by_govt, date_verified_by_govt) values('$pID', '1', now())";
	$result_1 = mysqli_query($link, $query_1);
	if($result_1)
		header("location: problem.php?pID=$pID");
	}
	...
	{
		$query_1 = "UPDATE Problem_solved set verified_by_govt='1', date_verified_by_govt=now() where pID='$pID'";
		$result_1 = mysqli_query($link, $query_1);
		if($result_1)
			header("location: problem.php?pID=$pID");
	}
